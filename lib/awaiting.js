 ...
 * function, returns a `Promise`. This `Promise` resolves if the first (error) argument of the
 ...
 * const promisifiedRead = a.promisify(readFile)
 * const contents = await promisifiedRead('foo.txt', 'utf-8')
 */
function promisified (fn) {
  return async function promisidiedCallback (...args) {
